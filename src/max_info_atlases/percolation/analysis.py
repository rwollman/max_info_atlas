...
def calculate_percolation_score(npz_file: Union[str, Path]) -> Tuple[Optional[Tuple[float, float]], Dict[str, float]]:
    ...
    Returns:
        Tuple of ((score, normalized_score), timing_dict) where scores are floats or None on error,
...
def aggregate_score_files_to_csv(
    ...
    for score_file in score_files:
        try:
            ...
            # v3 format (two values): score\tnormalized_score
            ...
            with open(score_file, 'r') as f:
                ...
                if '\t' in score_str:
                    ...
                    if len(parts) >= 4:
                        ...
                        # v3 format - score and normalized_score
                        score = float(parts[0])
